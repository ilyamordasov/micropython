...
def TX_setDefaultValues():
    ...
    radio.writeSingleByte(defs.PKTCTRL0,0x12) #Packet Automation Control
    ...
    radio.writeSingleByte(defs.FREND0,0x17)   #Front End TX Configuration
...
def rx_callback(code):
    #print ("{:<8} {:<15} {:<10} {:<10} {:<10}".format('Width','Period','Bit','Code','Count'))
    #for a in code:
    print(code)
        #(w, t, b, c, cnt) = a
        #print ("{:<8} {:<15} {:<10} 0x{:<10x} {:<10}".format(w,t,b,c,cnt))
        #print("%d %d %d 0x%x %d" % (w, t, b, c, cnt))
    #print("----------------------------------------------------------")

def tx_callback(code):
    print("Transmited: %s" % (code,))
